...
def persistent_streamk_gemm(
    ...
    while start_iter < last_iter:
        ...
        if start_iter != tile_iter:
            pm = pid * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
            pn = pid * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
            P_ = P + pm[:, None] + pn[None, :]
            tl.store(P_, acc)
            ...
            end = tile_iter + tl.minimum(last_iter, tile_iter_end) - tile_iter
            while (end < tile_iter_end) and (next_pid < num_sms):
                while tl.atomic_cas(locks + pid, 1, 1) != 1:
                    pass
                pm = next_pid * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
                pn = next_pid * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
                P_ = P + pm[:, None] + pn[None, :]
                acc1 = tl.load(P_)
                acc += acc1
            ...
            # After locally reducing the partial, write the tile:
            mask = (rm < M)[:, None] & (rn < N)[None, :]
            C_ = C + rm[:, None] * stride_cm + rn[None, :] * stride_cn
            tl.store(C_, acc, mask=mask)
